 ...
 *
 * A route handler rather than a Server Action: the Location is built
 * explicitly from the forwarded host, so it is testable and there is no hidden
 * origin check to misconfigure behind a proxy.
 */
export async function POST(request: Request) {
  ...
  if (typeof submitted !== "string" || !passwordMatches(submitted)) {
    ...
    if (next !== ADMIN_HOME) query.set("next", next);
