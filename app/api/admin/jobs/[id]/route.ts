...
import { businesses, scrapeJobs } from "@admin/lib/db/schema";
import { getApiUsage } from "@admin/lib/leads/stats";
import { logger } from "@admin/lib/log";
import { cancelJob, isActive, recoverInterruptedJobs } from "@admin/lib/scrape/runner";
import { summarizeJob, type JobFacts } from "@admin/lib/scrape/summary";
...
export async function GET(_request: Request, ctx: RouteContext<"/api/admin/jobs/[id]">) {
  ...
  // A job whose function was killed would otherwise poll as "running" forever.
  ...
  // From the row, not from memory: on Vercel this poll can land on a different
  // instance from the one running the job.
 ...
 */
export async function DELETE(_request: Request, ctx: RouteContext<"/api/admin/jobs/[id]">) {
  const { id } = await ctx.params;
  const jobId = Number(id);
  if (!Number.isInteger(jobId)) {
    return NextResponse.json({ error: "Invalid job id" }, { status: 400 });
  ...
  // Only reaches the job if it runs on this instance; the flag above is what
  // stops it anywhere else, between searches.
  cancelJob(jobId);
