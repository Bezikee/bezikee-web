...
import { businesses, leadEvents, leads } from "@admin/lib/db/schema";
import { scoreLead } from "@admin/lib/leads/classify";
import { isSellableFinding, judgeStatus, type ProbeVerdict } from "@admin/lib/leads/website-status";
...
/**
 * Probes run inside this request, and a batch of unresponsive hosts can take
 * minutes (two 8s timeouts each). Seconds, and a literal for the build.
 */
export const maxDuration = 300;

/**
 * Stop picking up new sites here, leaving time for the ones in flight to time
 * out and be written. Whatever is left stays `unchecked` and is reported as
 * `remaining`, so running the check again carries on from there.
 */
  ...
  for (const method of ["HEAD", "GET"] as const) {
    ...
    try {
      const res = await fetch(target, {
        ...
        redirect: "follow",
        headers: { "User-Agent": "Mozilla/5.0 (compatible; bezikee-link-check/1.0; +https://bezikee.com)" },
      });
