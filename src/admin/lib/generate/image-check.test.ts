...
import { findImageReference, findUnsupportedYear } from "./agent";
...
describe("findUnsupportedYear", () => {
  // The real payload shape: everything Google said, JSON-stringified.
  const source = JSON.stringify({
    reviews: [{ text: "sirviendo menús desde el año 1983, ahí es nada" }],
  });

  it("catches a year the data never mentions", () => {
    // Exactly what shipped: 1965 in the masthead, the title and the body, from
    // a business whose only stated year was 1983.
    expect(
      findUnsupportedYear(`<h1>Loreto</h1><p>Desde 1965, en Chamberí.</p>`, source),
    ).toBe("1965");
    expect(findUnsupportedYear(`<title>Loreto desde 1965</title>`, source)).toBe("1965");
  });

  it("allows a year the data does mention", () => {
    expect(findUnsupportedYear(`<p>Desde 1983.</p>`, source)).toBeNull();
  });

  it("ignores digits in CSS, which are not claims", () => {
    // Stylesheets are full of four-digit numbers; none of them are facts.
    const css = `<style>.a{font-weight:1900;z-index:2024;letter-spacing:0.1965em}</style><p>Hola</p>`;
    expect(findUnsupportedYear(css, source)).toBeNull();
  });

  it("ignores digits inside markup rather than text", () => {
    expect(
      findUnsupportedYear(`<div data-x="1965" class="c1999"><p>Hola</p></div>`, source),
    ).toBeNull();
  });

  it("reports the first unsupported year when there are several", () => {
    expect(findUnsupportedYear(`<p>1983 y 1965 y 1970</p>`, source)).toBe("1965");
  });
});
