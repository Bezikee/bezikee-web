import { COST_PER_REQUEST_USD, RESULTS_PER_REQUEST } from "@admin/lib/places/pricing";
...
function outcomeOf(facts: JobFacts): JobSummary["outcome"] {
  ...
  const truncated =
    ...
    Boolean(facts.stoppedReason?.includes("budget cap")) ||
    Boolean(facts.stoppedReason?.includes("time limit"));
...
export function summarizeJob(facts: JobFacts, context: JobContext): JobSummary {
  ...
  if (outcome === "failed") {
    recommendations.push({
      ...
      detail: `${facts.error ?? "No error recorded."} You were still billed for the ${
        ...
        2,
      )} list). The function logs in Vercel (or "npm run logs -- --errors" locally) have the API status and Google's error code.`,
    });
  }
  ...
  if (outcome === "interrupted") {
    recommendations.push({
      id: "interrupted",
      severity: "warning",
      title: "The run was cut off",
      detail:
        "The function running it stopped before it could finish. Everything found until then was saved, and the searches that completed are recorded as covered. Re-running picks up only what's left.",
      apply: {},
  ...
  // Serverless functions have a hard time limit, so a big sweep can take more
  // than one run. Nothing is lost: unfinished searches were never recorded as
  // covered, and the same settings pick up exactly where this one stopped.
  if (facts.stoppedReason?.includes("time limit") && remainingSearches > 0) {
    recommendations.push({
      id: "time_limit",
      severity: "warning",
      title: "Stopped at the time limit",
      detail:
        `${remainingSearches} search${remainingSearches === 1 ? "" : "es"} never ran. Re-run with the same ` +
        `settings — finished searches are skipped as covered, so it continues where this one stopped ` +
        `and you only pay for the remainder.`,
      apply: {},
      applyLabel: "Re-run the rest",
    });
  }
