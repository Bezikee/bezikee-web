...
describe("the time limit", () => {
  // Wide enough to tile into many cells, so there is work left when it stops.
  const WIDE: JobParams = { ...BASE, radius: 2000, cellRadius: 400, maxRequests: 1000 };

  it("stops cleanly and records only the searches that finished", async () => {
    stubFetch();
    // The clock jumps past the limit once a few requests are out — the same
    // thing a slow sweep does to a function with a hard time limit.
    const start = Date.now();
    vi.spyOn(Date, "now").mockImplementation(() =>
      requestCount >= 3 ? start + 60_000 : start,
    );

    const jobId = await createJob(WIDE);
    await runJob(jobId, WIDE, { timeLimitMs: 1000 });
    const job = await jobRow(jobId);

    // A clean stop, not a failure: it did what it was told and kept everything.
    expect(job.status).toBe("completed");
    expect(job.stoppedReason).toContain("time limit");
    expect(job.cellsDone).toBeGreaterThan(0);
    expect(job.cellsDone).toBeLessThan(job.cellsTotal);
    expect(job.leadsCreated).toBeGreaterThan(0);

    // Only finished searches count as covered; the rest must stay reachable.
    // (A search claimed and then abandoned at the limit is counted as done
    // but deliberately not covered, so this can be below cellsDone.)
    const covered = await db.select().from(cellCoverage);
    expect(covered.length).toBeGreaterThan(0);
    expect(covered.length).toBeLessThanOrEqual(job.cellsDone);
    expect(covered.length).toBeLessThan(job.cellsTotal);
  });

  it("lets a re-run pick up exactly where it stopped", async () => {
    stubFetch();
    const start = Date.now();
    const clock = vi
      .spyOn(Date, "now")
      .mockImplementation(() => (requestCount >= 3 ? start + 60_000 : start));

    const firstId = await createJob(WIDE);
    await runJob(firstId, WIDE, { timeLimitMs: 1000 });
    const first = await jobRow(firstId);
    const coveredByFirst = (await db.select().from(cellCoverage)).length;
    clock.mockRestore();

    const secondId = await createJob(WIDE);
    await runJob(secondId, WIDE, { timeLimitMs: 60_000 });
    const second = await jobRow(secondId);

    expect(second.status).toBe("completed");
    expect(second.stoppedReason).toBeNull();
    // Everything the first run finished is skipped rather than bought twice…
    expect(second.cellsSkipped).toBe(coveredByFirst);
    // …and between them every search in the area has been made exactly once.
    expect(await db.select().from(cellCoverage)).toHaveLength(first.cellsTotal);
  });

  it("has no effect when the run fits inside it", async () => {
    stubFetch();
    const jobId = await createJob(BASE);
    await runJob(jobId, BASE, { timeLimitMs: 60_000 });
...
describe("recoverInterruptedJobs", () => {
  /** A job row as a killed function would have left it. */
  async function jobCreated(status: "pending" | "running" | "completed", ageMs: number) {
    const [row] = await db
      .insert(scrapeJobs)
      .values({
        areaName: "Test Area",
  ...
  const LIMIT_MS = SCRAPE_MAX_DURATION_SECONDS * 1000;

  it("marks jobs older than any function could live as interrupted", async () => {
    const deadRunning = await jobCreated("running", LIMIT_MS * 2);
    const deadPending = await jobCreated("pending", LIMIT_MS * 2);
    ...
    const rows = await db
      .select()
      .from(scrapeJobs)
      .where(inArray(scrapeJobs.id, [deadRunning, deadPending]));
    expect(rows.map((row) => row.status)).toEqual(["interrupted", "interrupted"]);
    expect(rows[0].finishedAt).not.toBeNull();
  });

  it("leaves a job that may still be running on another instance alone", async () => {
    // A cold start must not kill work it can't see. On a server, "recover at
    // boot" was safe; on Vercel another instance may be mid-scrape.
    const live = await jobCreated("running", 10_000);

    expect(await recoverInterruptedJobs()).toBe(0);
    expect((await jobRow(live)).status).toBe("running");
  });

  it("never touches finished jobs, however old", async () => {
    const done = await jobCreated("completed", LIMIT_MS * 10);

    await recoverInterruptedJobs();
    expect((await jobRow(done)).status).toBe("completed");
  });
});

describe("hasRunningJob", () => {
  it("is true while a job is in progress", async () => {
    await createJob(BASE);
    expect(await hasRunningJob()).toBe(true);
  });

  it("is false once the only running job is one a killed function left behind", async () => {
    await db.insert(scrapeJobs).values({
      areaName: "Test Area",
      params: "{}",
      status: "running",
      createdAt: new Date(Date.now() - SCRAPE_MAX_DURATION_SECONDS * 2000),
    });

    // Otherwise one killed function would block every scrape after it.
    expect(await hasRunningJob()).toBe(false);
  });
