...
describe("time limit", () => {
  const STOPPED = {
    stoppedReason: "Stopped at the 5-minute time limit.",
    cellsTotal: 75,
    cellsDone: 40,
  };

  it("reports a partial run, not a finished one", () => {
    expect(summarizeJob({ ...CLEAN, ...STOPPED }, QUIET).outcome).toBe("partial");
  });

  it("offers to carry on with the same settings rather than a bigger budget", () => {
    // The budget was fine; the function ran out of time. Raising the cap would
    // change nothing, while a plain re-run skips what finished and continues.
    const rec = find("time_limit", STOPPED)!;
    expect(rec.detail).toContain("35 searches never ran");
    expect(rec.apply).toEqual({});
    expect(ids(STOPPED)).not.toContain("budget");
  });

  it("stays quiet when the run finished just as time ran out", () => {
    expect(ids({ ...STOPPED, cellsDone: 75 })).not.toContain("time_limit");
  });
});
...
describe("failures", () => {
  it("reports the error and what it still cost", () => {
    ...
    expect(rec.detail).toContain("function logs in Vercel");
  });
