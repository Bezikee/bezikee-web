...
describe("safeNext", () => {
  it("keeps a destination inside the admin panel", () => {
    expect(safeNext("/admin/")).toBe("/admin/");
    expect(safeNext("/admin")).toBe("/admin");
    expect(safeNext("/admin/leads/12/")).toBe("/admin/leads/12/");
    expect(safeNext("/admin/leads/?status=new")).toBe("/admin/leads/?status=new");
  });

  it("refuses to send a fresh session anywhere else", () => {
    for (const bad of [
      ...
      expect(safeNext(bad), String(bad)).toBe("/admin/");
    }
  });
});
