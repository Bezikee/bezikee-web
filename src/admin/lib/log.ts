...
export function redact(value: unknown, seen = new WeakSet<object>()): unknown {
  ...
  if (value instanceof Error) {
    return {
      ...
      // Typed by hand: this project's `lib` (ES2020) predates Error.cause.
      ...((value as Error & { cause?: unknown }).cause
        ? { cause: redact((value as Error & { cause?: unknown }).cause, seen) }
        : {}),
...
function envOptions(): LoggerOptions {
  ...
  return {
    // Locally, JSONL files under data/logs for `npm run logs`. On Vercel the
    // filesystem is read-only and not kept between invocations, so output is
    // console only — Vercel captures it in the project's Logs tab.
    dir: process.env.LOG_DIR ?? (process.env.VERCEL ? "" : path.join("data", "logs")),
 ...
 */
const globalForLog = globalThis as unknown as { __bezikeeAdminLog?: LoggerFactory };

export const logger: LoggerFactory = (globalForLog.__bezikeeAdminLog ??=
  createLoggerFactory());
