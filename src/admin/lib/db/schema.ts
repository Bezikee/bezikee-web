 ...
 * nothing but Claude tokens. The page itself is published to `demo_sites`.
 */
export const siteBuilds = pgTable(
  "site_builds",
  {
    ...
    /** Name of the build's scratch directory. See `src/admin/lib/generate/paths.ts`. */
...
/**
 * The published demo page for a business, served at demo.bezikee.com/<id>.
 *
 * `id` is a random v4 UUID and is the only thing standing between the page and
 * the public, so it is never derived from anything guessable. One row per
 * business: a rebuild replaces the HTML and keeps the id, so a link that has
 * already been sent to the owner keeps working. See `src/admin/lib/demo`.
 */
export const demoSites = pgTable(
  "demo_sites",
  {
    ...
    businessId: integer("business_id")
      .notNull()
      .references(() => businesses.id, { onDelete: "cascade" }),
    /** The build that produced the current page. Null for imported pages. */
    buildId: integer("build_id").references(() => siteBuilds.id, { onDelete: "set null" }),
    /** One self-contained file: inline CSS, inline SVG, no scripts, no images. */
    html: text("html").notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => [uniqueIndex("demo_sites_business_id_idx").on(t.businessId)],
);
