 ...
 *
 * Behind a proxy the server's own view of the request can name an internal
 * address rather than the one the browser used, and a redirect built from it
 * sends the browser somewhere it cannot reach. Vercel's edge terminates TLS and
 * forwards the real host in `x-forwarded-*`, so that is what a redirect has to
 * be built from.
 *
 * Falls back to `host` for direct connections (local development), and finally
 * to a relative URL if neither header is present.
 */
