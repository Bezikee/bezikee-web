...
function GlobeIcon() {
  return (
    <svg viewBox="0 0 16 16" aria-hidden className="size-4" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <path d="M2 8h12M8 2c1.6 1.7 2.5 3.8 2.5 6S9.6 12.3 8 14c-1.6-1.7-2.5-3.8-2.5-6S6.4 3.7 8 2Z" />
    </svg>
  );
}
              ...
              return (
                ...
                >
                  <td className="px-4 py-2.5">
                    <Link href={`/admin/leads/${row.leadId}/`} className="block">
                  ...
                  <td className="px-4 py-2.5">
                    <div
                      className={`flex items-center justify-end gap-0.5 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100 ${
                        row.demoUrl ? "opacity-60" : "opacity-0"
                      }`}
                    >
                      {/* Always visible, unlike the rest: whether a demo
                          exists is a property of the lead worth seeing while
                          scanning, not an action to reach for on hover. */}
                      {row.demoUrl ? (
                        <a
                          href={row.demoUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          title="Open the generated demo site"
                          aria-label={`Demo site for ${row.name}`}
                          className={`${ACTION_CLASS} text-accent hover:text-accent-hover`}
                        >
                          <GlobeIcon />
                        </a>
                      ) : null}
