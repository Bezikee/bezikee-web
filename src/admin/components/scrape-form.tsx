...
import { AREAS, getArea } from "@admin/config/areas";
import { CATEGORIES, CATEGORY_GROUPS, type Category } from "@admin/config/categories";
import { COST_PER_REQUEST_USD, RESULTS_PER_REQUEST } from "@admin/lib/places/pricing";
import { CELL_SIZES, defaultCellRadius } from "@admin/lib/scrape/params";
import type { JobSummary, SuggestedParams } from "@admin/lib/scrape/summary";
...
export function ScrapeForm({ hasApiKey }: { hasApiKey: boolean }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setEstimating(true);
      try {
        const res = await fetch("/api/admin/jobs/estimate/", {
          method: "POST",
      ...
      {!hasApiKey ? (
        <div className="rounded-xl border border-line bg-card-muted px-4 py-3 text-sm">
          ...
          <span className="text-ink-secondary">
            Set <code className="font-mono text-xs">GOOGLE_MAPS_API_KEY</code> in the
            Vercel project (or <code className="font-mono text-xs">.env.local</code>{" "}
            locally) and redeploy. You can still price a sweep below without one.
          </span>
        </div>
      ) : null}
