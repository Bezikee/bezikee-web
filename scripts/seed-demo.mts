 ...
 * which is why it refuses anything but a local database unless you pass
 * --allow-remote. `--reset` deletes every row, real or not.
 */
import "./load-env.mjs";
...
import { classifyWebsite, scoreLead } from "@admin/lib/leads/classify";
...
// A laptop pointed at production is a normal setup here (it is how demo sites
// get generated), so the dangerous case is one typo away.
if (!isLocal(DATABASE_URL) && !process.argv.includes("--allow-remote")) {
  throw new Error(
    "DATABASE_URL is not a local database. seed:demo writes fake businesses (and --reset deletes everything), so it refuses to run against a remote one. Pass --allow-remote if you really mean it.",
  );
}
...
console.log("Next:  npm run dev     then open http://localhost:3000/admin/");
